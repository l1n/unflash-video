...
- <!-- 16:00 --> **Sound in the section player**, off to start with: turn it on with *🔇 sound off* under the player. It plays at 1×, and held frames are silent there too, as in the export.
- <!-- 16:00 --> **Back to the whole video, charted.** *Whole video*, at the top of the sections list, opens the whole video again, with its flashing charted around the playhead (10 s, 30 s, 2 min or all of it): bars for flashing faster than the limit, a blue line for flashing at the limit rate (what extended flashes are made of), brightness and stripes, under the limit too. Point at it for the numbers; click to play from there.
...
- <!-- 16:00 --> **Extended flashes you can find and fix.** A section's check now lists what still fails (each flash, red flash, extended flash, stripes) with its frames and times, a button to select those frames and how to fix it. *Select unsafe frames* includes extended flashes and scrolls to them, and frames inside an extended flash have a blue outline.
- <!-- 16:00 --> **Project files.** *Project…* in the header saves the sections, marks and scan to a file and loads one back: on another computer or browser, or after the site's data is cleared. The project kept in the browser also comes back when the same video is opened again as a new copy (the same name and size).
